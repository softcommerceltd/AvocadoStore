...
class ShipmentCreateManagement extends AbstractManagement implements ShipmentCreateManagementInterface
{
     ...
     */
    private ShipmentRepositoryInterface $_shipmentRepository;
    ...
    public function execute()
    {
        ...
        $searchCriteria = $this->_searchCriteriaBuilder
            ->addFilter(OrderInterface::STATUS, Status::PENDING)
            ->create()
            ->setCurrentPage($currentPage)
            ->setPageSize($pageSize);

        $collection = $this->_shipmentRepository->getList($searchCriteria);
    ...
    private function _processMultiple(array $clientOrders)
    {
        ...
        foreach ($clientOrders as $clientOrder) {
            try {
                ...
                $this->_shipmentRepository->save($this->_getClientOrder());
    ...
    private function _process(OrderInterface $clientOrder)
    {
        ...
        try {
            ...
            $this->addResponse(
                __('Could not create order. [Order: %1, Reason: %1]', $clientOrder->getAvocadoOrderId(), $e->getMessage()),
                Status::ERROR
            );
        }
    ...
    private function _createOrder()
    {
        ...
        $quote->getPayment()->importData(
            [
                PaymentInterface::KEY_METHOD => 'purchaseorder',
